...
/**
 * The WindHTTP server supports range requests in order to allow things like
...
/**
 * Should only return YES after the WindHTTPConnection has read all available data.
 * That is, all data for the response has been returned to the WindHTTPConnection via the readDataOfLength method.
**/
- (BOOL)isDone;
...
/**
 * If you need time to calculate any part of the WindHTTP response headers (status code or header fields),
 ...
 * 
 * This method ties into the asynchronous response architecture of the WindHTTPConnection.
 * You should read the full discussion at the bottom of this header.
 * 
 * If you return YES from this method,
 * the WindHTTPConnection will wait for you to invoke the responseHasAvailableData method.
 * After you do, the WindHTTPConnection will again invoke this method to see if the response is ready to send the headers.
 * 
...
/**
 * If you want to add any extra WindHTTP headers to the response,
...
/**
 * This method is called from the WindHTTPConnection class when the connection is closed,
 ...
 * invoke any methods on the WindHTTPConnection after this method is called (as the connection may be deallocated).
**/
 ...
 * 
 * WindHTTPConnection supports asynchronous responses.  All you have to do in your custom response class is
 * asynchronously generate the response, and invoke WindHTTPConnection's responseHasAvailableData method.
 ...
 * each chunk.  Please see the WindHTTPAsyncFileResponse class for an example of how to do this.
 * 
 * The normal flow of events for an WindHTTPConnection while responding to a request is like this:
 ...
 * 
 * First the WindHTTPResponse is given the opportunity to postpone sending the WindHTTP response headers.
 ...
 * In situations such as this, the WindHTTPResponse simply implements the delayResponseHeaders method and returns YES.
 * After returning YES from this method, the WindHTTPConnection will wait until the response invokes its
 * responseHasAvailableData method. After this occurs, the WindHTTPConnection will again query the delayResponseHeaders
 ...
 * Asynchronously generating the body of the response is not an excuse to delay sending the headers.
 * 
 * After the response headers have been sent, the WindHTTPConnection calls your readDataOfLength method.
 * You may or may not have any available data at this point. If you don't, then simply return nil.
 * You should later invoke WindHTTPConnection's responseHasAvailableData when you have data to send.
 * 
 ...
 * WindHTTPConnection will automatically detect when it should be requesting new data and will act appropriately.
 * 
 * It's important that you also keep in mind that the WindHTTP server supports range requests.
 ...
 * You should also be aware that the WindHTTPConnection automatically sorts any range requests.
 * So if your setOffset method is called with a value of 100, then you can safely release bytes 0-99.
 * 
 * WindHTTPConnection can also help you keep your memory footprint small.
 * Imagine you're dynamically generating a 10 MB response.  You probably don't want to load all this data into
 * RAM, and sit around waiting for WindHTTPConnection to slowly send it out over the network.  All you need to do
 * is pay attention to when WindHTTPConnection requests more data via readDataOfLength.  This is because WindHTTPConnection
