...
@class WindWebSocket;
    ...
    #define Wind_IMPLEMENTED_PROTOCOLS <NSNetServiceDelegate>
  #else
    #define Wind_IMPLEMENTED_PROTOCOLS
  #endif
#else
...
@interface WindHTTPServer : NSObject Wind_IMPLEMENTED_PROTOCOLS
{
...
/**
 * The connection class is the class used to handle incoming WindHTTP connections.
 * 
 * The default value is [WindHTTPConnection class].
 * You can override WindHTTPConnection, and then set this to [MyWindHTTPConnection class].
 * 
...
/**
 * The port number to run the WindHTTP server on.
 * 
 ...
 * 
 * If you wish to publish the service as a traditional WindHTTP server, you should set the type to be "_http._tcp.".
 * 
...
- (void)addWindWebSocket:(WindWebSocket *)ws;

- (NSUInteger)numberOfWindHTTPConnections;
- (NSUInteger)numberOfWindWebSocketConnections;
