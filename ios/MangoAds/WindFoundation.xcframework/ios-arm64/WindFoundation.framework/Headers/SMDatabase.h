...
NS_ASSUME_NONNULL_BEGIN
    ...
    #define SMDBAutorelease(__v) ([__v autorelease]);
    #define SMDBReturnAutoreleased SMDBAutorelease

    #define SMDBRetain(__v) ([__v retain]);
    #define SMDBReturnRetained SMDBRetain

    #define SMDBRelease(__v) ([__v release]);

    #define SMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define SMDBAutorelease(__v)
    #define SMDBReturnAutoreleased(__v) (__v)

    #define SMDBRetain(__v)
    #define SMDBReturnRetained(__v) (__v)

    #define SMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define SMDBDispatchQueueRelease(__v)
    #else
        #define SMDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
 ...
 */
typedef int(^SMDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);

/**
 Enumeration used in checkpoint methods.
 */
typedef NS_ENUM(int, SMDBCheckpointMode) {
    SMDBCheckpointModePassive  = 0, // SQLITE_CHECKPOINT_PASSIVE,
    SMDBCheckpointModeFull     = 1, // SQLITE_CHECKPOINT_FULL,
    SMDBCheckpointModeRestart  = 2, // SQLITE_CHECKPOINT_RESTART,
    SMDBCheckpointModeTruncate = 3  // SQLITE_CHECKPOINT_TRUNCATE
};
...
/** A SQLite ([https://sqlite.org/](https://sqlite.org/)) Objective-C wrapper.
 ...
 The three main classes in SMDB are:

 - @c SMDatabase - Represents a single SQLite database.  Used for executing SQL statements.

 - @c SMResultSet - Represents the results of executing a query on an @c SMDatabase .

 - @c SMDatabaseQueue - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 See also

 - @c SMDatabasePool - A pool of @c SMDatabase objects

 - @c SMStatement - A wrapper for @c sqlite_stmt

 External links

 - [SMDB on GitHub](https://github.com/ccgus/fmdb) including introductory documentation
 - [SQLite web site](https://sqlite.org/)
 - [SMDB mailing list](http://groups.google.com/group/fmdb)
 - [SQLite FAQ](https://sqlite.org/faq.html)

 @warning Do not instantiate a single @c SMDatabase  object and use it across multiple threads. Instead, use @c SMDatabaseQueue .
...
/** Create a @c SMDatabase  object.
 
 An @c SMDatabase  is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.

 2. An zero-length string.  An empty database is created at a temporary location.  This database is deleted with the @c SMDatabase  connection is closed.

 3. @c nil .  An in-memory database is created.  This database will be destroyed with the @c SMDatabase  connection is closed.
...
SMDatabase *db = [SMDatabase databaseWithPath:fileURL.path];
@endcode
 ...
 @return @c SMDatabase  object if successful; @c nil  if failure.
...
/** Create a @c SMDatabase  object.
 ...
 2. @c nil .  An in-memory database is created.  This database will be destroyed with the @c SMDatabase  connection is closed.
...
NSURL *folder  = [[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:true error:&error];
NSURL *fileURL = [folder URLByAppendingPathComponent:@"test.db"];
SMDatabase *db = [SMDatabase databaseWithURL:fileURL];
@endcode
...
/** Initialize a @c SMDatabase  object.
 
 An @c SMDatabase  is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.

 2. A zero-length string.  An empty database is created at a temporary location.  This database is deleted with the @c SMDatabase  connection is closed.
 ...
 NSURL *folder  = [[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:true error:&error];
 NSURL *fileURL = [folder URLByAppendingPathComponent:@"test.db"];
 SMDatabase *db = [[SMDatabase alloc] initWithPath:fileURL.path];
 @endcode
...
/** Initialize a @c SMDatabase  object.
 ...
 An @c SMDatabase  is created with a local file URL to a SQLite database file.  This path can be one of these three:
 ...
 NSURL *folder  = [[NSFileManager defaultManager] URLForDirectory:NSApplicationSupportDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:true error:&error];
 NSURL *fileURL = [folder URLByAppendingPathComponent:@"test.db"];
 SMDatabase *db = [[SMDatabase alloc] initWithURL:fileURL];
 @endcode
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) SMDBExecuteStatementsCallbackBlock _Nullable)block;
...
/** Execute select statement
 ...
 Executing queries returns an @c SMResultSet  object if successful, and @c nil  upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the @c lastErrorMessage  and @c lastErrorMessage  methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[SMResultSet next]>`) from one record to the other.
 ...
 @return A @c SMResultSet  for the result set upon success; @c nil  upon failure. If failed, you can call @c lastError , @c lastErrorCode , or @c lastErrorMessage  for diagnostic information regarding the failure.
 
 @see SMResultSet
 @see [`SMResultSet next`](<[SMResultSet next]>)
 @see [`sqlite3_bind`](https://sqlite.org/c3ref/bind_blob.html)
...
- (SMResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (SMResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (SMResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
/** Execute select statement
 ...
 `func executeQuery(sql: String, values: [Any]?) throws  -> SMResultSet!`
...
- (SMResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (SMResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (SMResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;

/// Prepare SQL statement.
///
/// @param sql SQL statement to prepare, generally with `?` placeholders.

- (SMResultSet *)prepare:(NSString *)sql;
...
/// The SMDB version number as a string in the form of @c "2.7.7" .
///
/// If you want to compare version number strings, you can use NSNumericSearch option:
///
/// @code
/// NSComparisonResult result = [[SMDatabase SMDBUserVersion] compare:@"2.11.0" options:NSNumericSearch];
/// @endcode
///
/// @returns The version number string.

+ (NSString*)SMDBUserVersion;

/** The SMDB version

 This returns the SMDB as hexadecimal value, e.g., @c 0x0243 for version 2.4.3.
 ...
 @warning This routine will not work if any component of the version number exceeds 15.
       ...
       For this reason, we would recommend using @c SMDBUserVersion  and with @c NSNumericSearch  option, e.g.
...
+ (SInt32)SMDBVersion __deprecated_msg("Use SMDBUserVersion instead");
...
typedef NS_ENUM(int, SMSqliteValueType) {
    SMSqliteValueTypeInteger = 1,
    SMSqliteValueTypeFloat   = 2,
    SMSqliteValueTypeText    = 3,
    SMSqliteValueTypeBlob    = 4,
    SMSqliteValueTypeNull    = 5
};

- (SMSqliteValueType)valueType:(void *)argv;
...
@code
myDB.dateFormat = [SMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
@endcode
 ...
 @warning Note that @c NSDateFormatter  is not thread-safe, so the formatter generated by this method should be assigned to only one SMDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using @c SMDatabase:storeableDateFormat .
 ...
 @warning Note there is no direct getter for the @c NSDateFormatter , and you should not use the formatter you pass to SMDB for other purposes, as @c NSDateFormatter  is not thread-safe.
 */
...
/** Objective-C wrapper for @c sqlite3_stmt
 ...
 This is a wrapper for a SQLite @c sqlite3_stmt . Generally when using SMDB you will not need to interact directly with `SMStatement`, but rather with @c SMDatabase  and @c SMResultSet  only.
...
@interface SMStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
