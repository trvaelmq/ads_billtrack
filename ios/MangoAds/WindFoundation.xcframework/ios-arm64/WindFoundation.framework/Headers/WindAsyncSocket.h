...
@class WindAsyncReadPacket;
@class WindAsyncWritePacket;
@class WindAsyncSocketPreBuffer;
@protocol WindAsyncSocketDelegate;

NS_ASSUME_NONNULL_BEGIN

extern NSString *const WindAsyncSocketException;
extern NSString *const WindAsyncSocketErrorDomain;

extern NSString *const WindAsyncSocketQueueName;
extern NSString *const WindAsyncSocketThreadName;

extern NSString *const WindAsyncSocketManuallyEvaluateTrust;
#if TARGET_OS_IPHONE
extern NSString *const WindAsyncSocketUseCFStreamForTLS;
#endif
#define WindAsyncSocketSSLPeerName     (NSString *)kCFStreamSSLPeerName
#define WindAsyncSocketSSLCertificates (NSString *)kCFStreamSSLCertificates
#define WindAsyncSocketSSLIsServer     (NSString *)kCFStreamSSLIsServer
extern NSString *const WindAsyncSocketSSLPeerID;
extern NSString *const WindAsyncSocketSSLProtocolVersionMin;
extern NSString *const WindAsyncSocketSSLProtocolVersionMax;
extern NSString *const WindAsyncSocketSSLSessionOptionFalseStart;
extern NSString *const WindAsyncSocketSSLSessionOptionSendOneByteRecord;
extern NSString *const WindAsyncSocketSSLCipherSuites;
extern NSString *const WindAsyncSocketSSLALPN;
#if !TARGET_OS_IPHONE
extern NSString *const WindAsyncSocketSSLDiffieHellmanParameters;
#endif

#define WindAsyncSocketLoggingContext 65535


typedef NS_ERROR_ENUM(WindAsyncSocketErrorDomain, WindAsyncSocketError) {
	WindAsyncSocketNoError = 0,           // Never used
	WindAsyncSocketBadConfigError,        // Invalid configuration
	WindAsyncSocketBadParamError,         // Invalid parameter was passed
	WindAsyncSocketConnectTimeoutError,   // A connect operation timed out
	WindAsyncSocketReadTimeoutError,      // A read operation timed out
	WindAsyncSocketWriteTimeoutError,     // A write operation timed out
	WindAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	WindAsyncSocketClosedError,           // The remote peer closed the connection
	WindAsyncSocketOtherError,            // Description provided in userInfo
};
...
@interface WindAsyncSocket : NSObject

/**
 * WindAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, WindAsyncSocket will automatically create it's own socket queue.
...
- (instancetype)initWithDelegate:(nullable id<WindAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq;
- (instancetype)initWithDelegate:(nullable id<WindAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq NS_DESIGNATED_INITIALIZER;

/**
 * Create WindAsyncSocket from already connect BSD socket file descriptor
**/
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<WindAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq error:(NSError**)error;

+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<WindAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError **)error;

#pragma mark Configuration

@property (atomic, weak, readwrite, nullable) id<WindAsyncSocketDelegate> delegate;
...
- (void)getDelegate:(id<WindAsyncSocketDelegate> __nullable * __nullable)delegatePtr delegateQueue:(dispatch_queue_t __nullable * __nullable)delegateQueuePtr;
- (void)setDelegate:(nullable id<WindAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
...
**/
- (void)synchronouslySetDelegate:(nullable id<WindAsyncSocketDelegate>)delegate;
- (void)synchronouslySetDelegateQueue:(nullable dispatch_queue_t)delegateQueue;
- (void)synchronouslySetDelegate:(nullable id<WindAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;

/**
 * By default, both IPv4 and IPv6 are enabled.
 * 
 * For accepting incoming connections, this means WindAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means WindAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, WindAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, WindAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of WindAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a WindAsyncSocket instance (e.g. in a dealloc method)
 ...
 * it is treated similarly to a timeout - the socket is closed with a WindAsyncSocketReadMaxedOutError.
 ...
 * This is due to the fact that WindAsyncSocket does NOT copy the data. It simply retains it.
 ...
 * 
 * - WindAsyncSocketManuallyEvaluateTrust
 ...
 *     So WindAsyncSocket will invoke the delegate method socket:shouldTrustPeer: passing the SecTrustRef.
 *
 ...
 *
 * - WindAsyncSocketUseCFStreamForTLS (iOS only)
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     By default WindAsyncSocket will use the SecureTransport layer to perform encryption.
 ...
 *     
 *     However, if you absolutely must, you can instruct WindAsyncSocket to use the old-fashioned encryption
 *     technique by going through the CFStream instead. So instead of using SecureTransport, WindAsyncSocket
 ...
 *     
 *     Thus all the other keys in the given dictionary will be ignored by WindAsyncSocket,
 ...
 *
 * - WindAsyncSocketSSLProtocolVersionMin
 * - WindAsyncSocketSSLProtocolVersionMax
 ...
 *
 * - WindAsyncSocketSSLDiffieHellmanParameters (Mac OS X only)
 *     The value must be of type NSData.
 ...
 *     You MUST use manual trust evaluation instead (see WindAsyncSocketManuallyEvaluateTrust).
 ...
 *     You MUST use WindAsyncSocketSSLProtocolVersionMin & WindAsyncSocketSSLProtocolVersionMin instead.
...
/**
 * WindAsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(WindAsyncSocket *)sock
 * {
 ...
 *     return socketQueue;
 * }
 * - (void)socket:(WindAsyncSocket *)sock didAcceptNewSocket:(WindAsyncSocket *)newSocket
 * {
 ...
 * Example usage:
 * 
 * - (void)socket:(WindAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port
 * {
...
@protocol WindAsyncSocketDelegate <NSObject>
@optional
...
**/
- (nullable dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(WindAsyncSocket *)sock;
 ...
 * The host parameter will be an IP address, not a DNS name.
 **/
- (void)socket:(WindAsyncSocket *)sock didConnectToUrl:(NSURL *)url;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)socket:(WindAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)socket:(WindAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(WindAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used for things such as updating progress bars.
**/
- (void)socket:(WindAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(WindAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(WindAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(WindAsyncSocket *)sock;
 ...
 * 
 * Note: If the WindAsyncSocket instance is deallocated while it is still connected,
...
**/
- (void)socketDidDisconnect:(WindAsyncSocket *)sock withError:(nullable NSError *)err;
...
**/
- (void)socket:(WindAsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust
